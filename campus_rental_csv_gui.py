...
# >>> Change CSV folder here or via env var UMBRELLA_DATA_DIR <<<
DATA_DIR = os.path.abspath(os.getenv(
    "UMBRELLA_DATA_DIR",
    "C:/Users/zone0/Desktop/Homework/INT6127/Data_csv"
))
...
STUDENTS_HEADERS = ["sid", "name", "email", "pwd_hash", "is_active", "created_at"]
STAFF_HEADERS    = ["staff_id", "name", "email", "role", "pwd_hash", "is_active", "created_at"]
...
def ensure_data_dir_and_seed():
    ...
    if not os.path.exists(STAFF_CSV):
        write_csv(STAFF_CSV, STAFF_HEADERS, [{
            ...
            "role": "clerk",
            "pwd_hash": hash_password("staff123"),
            "is_active": "1",
            "created_at": now_str(),
        }])
...
def login_student(sid, pwd):
    ...
    if not s or s.get("is_active") != "1":
        return None
    if s["pwd_hash"] != hash_password(pwd):
        return None
    return s
...
class App(tk.Tk):
    ...
    def _on_login_success(self, role, user):
        ...
        if role == "student":
            ...
            StaffUI(container, user, on_logout=self._show_login).pack(fill="both", expand=True)
